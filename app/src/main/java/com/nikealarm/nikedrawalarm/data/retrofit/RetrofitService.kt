...
interface RetrofitService {
    ...
    suspend fun getFeedProductData(
        @Query("anchor") anchor: Int,
        @Query("count") count: Int = 50
    ): ProductDTO

    @GET("?filter=marketplace%28KR%29&filter=language%28ko%29&filter=upcoming%28true%29&filter=channelId%28010794e5-35fe-4e32-aaff-cd2c74f89d61%29&filter=exclusiveAccess%28true%2Cfalse%29")
    suspend fun getUpcomingProductData(
        @Query("anchor") anchor: Int,
        @Query("count") count: Int = 50
    ): ProductDTO
}